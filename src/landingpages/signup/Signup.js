    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await validationSchema.validate({
                ...
                email,
                password,
            }, { abortEarly: false }); // abortEarly: false will ensure all validations are run and not stop at the first error
            ...
            <div className='log-div'>
                ...
                <div className='log_div3'>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='head-email'>
                            <p className='email'>Name</p>
                            <input type='text' placeholder='Enter Your Full name' className='main-email' value={name} onChange={(e) => setName(e.target.value)} />
                            {errors.name && <span className="error">{errors.name}</span>}
                        </div>
                        <div className='head-email'>
                            <p className='email'>Phone</p>
                            <PhoneInput placeholder='Enter Your Phone no' value={phone} onChange={setPhone} className='main-email' />
                            {errors.phone && <span className="error">{errors.phone}</span>}
                        </div>
                        <div className='head-email'>
                            <p className='email'>Email</p>
                            <input type='email' placeholder='Enter Your Email' className='main-email' value={email} onChange={(e) => setEmail(e.target.value)} />
                            {errors.email && <span className="error">{errors.email}</span>}
                        </div>
                        <div className='head-email'>
                            <p className='email'>Password</p>
                            <div className='main-email-1'>
                                <input type={showPassword ? "text" : "password"} placeholder='Enter Your Password' className='input-2' value={password} onChange={(e) => setPassword(e.target.value)} />
                                <span onClick={() => setShowPassword(!showPassword)}>
                    ...
                    <div className='account'>
                        <p className='para1'>Already have an account?<span style={{ color: "blue", cursor:"pointer" }}  onClick={() => login1()}> Sign in</span></p>
                    </div>
                    <div className='orange'>
                        <p>OR</p>
                    </div>
